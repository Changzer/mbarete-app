...
import { orders, orderItems, orderDocuments, orderEvents } from "@/db/schema";
...
export async function storeDossierDocument(
  ...
  try {
    storedPath = item.video
      ...
      : await saveUploadedDossierDocument(user.companyId, file);
  } catch (err) {
  ...
  try {
    // The file row and history either both commit or both roll back. An audit
    // failure must not report a failed upload after the document was saved.
    await db.transaction(async (tx) => {
      await tx.insert(orderDocuments).values({
      ...
      await tx.insert(orderEvents).values({
        companyId: user.companyId, orderId, userId: user.id, kind: "document_added",
        payload: JSON.stringify({ kind, fapiaoType: fapiao, name: file.name || "document" }),
      });
    });
  } catch (error) {
    // storedPath was generated by the saver, never supplied by the caller.
    await fs.unlink(path.join(/* turbopackIgnore: true */ uploadsDir(), storedPath.slice("/uploads/".length)))
      .catch((cleanupError) => console.error("Failed to remove uncommitted dossier file", cleanupError));
    throw error;
  }
  return {};
}
