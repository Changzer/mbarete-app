...
test("a booth capture becomes a product, an order, a quote and an invoice", async () => {
  ...
  try {
    ...
    await page.reload();
    assert.match(await page.getByTestId(`supplier-group-${supplierA}`).filter({ visible: true }).innerText(), /Original Factory/);
    ...
    assert.match(await refreshDialog.innerText(), /Original Factory/);
